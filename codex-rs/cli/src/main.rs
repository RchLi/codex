...
#[clap(
    ...
    // the generic `elva` command name that users run.
    bin_name = "elva",
    override_usage = "elva [OPTIONS] [PROMPT]\n       elva [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Elva.
    Mcp(McpCli),

    /// Manage Elva plugins.
    Plugin(PluginCli),

    /// Start Elva as an MCP server (stdio).
    ...
    /// Update Elva to the latest version.
    Update,

    /// Diagnose local Elva installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Elva-provided sandbox.
    ...
    /// Apply the latest diff produced by Elva agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Elva Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Elva.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Elva tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Elva via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Elva.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`elva update` is not available in debug builds. Install a release build of Elva to use this command."
        );
    }
    ...
    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Elva installation method. Please update manually: https://developers.openai.com/codex/cli/"
            );
        };
        run_update_action(action)
    }
    ...
    match subcommand {
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`elva sandbox` is not supported on this operating system");
            }
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `elva mcp`: `elva`, `elva exec`, `elva review`, `elva resume`, `elva archive`, `elva delete`, `elva unarchive`, `elva fork`, `elva mcp`, `elva sandbox`, and `elva debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let codex_self_exe = arg0_paths
        .codex_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Elva executable path is not configured"))?;
...
async fn load_exec_server_remote_auth_provider(
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `elva login` or set CODEX_API_KEY",
    )
    .await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `elva {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `elva {subcommand}`"
        );
    }
    Ok(())
}
...
/// `elva --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `elva {subcommand}`");
    }
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Elva's interactive TUI may not work in this terminal."
        );
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Elva local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
/// Build the final `TuiCli` for a `elva resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `elva` without additional flags.
...
/// Build the final `TuiCli` for a `elva fork` invocation.
...
mod tests {
    ...
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let codex_home = tempfile::tempdir().expect("temporary Elva home");
        let config = ConfigBuilder::default()
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        ...
        assert!(
            help.contains("Usage: elva plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: elva plugin marketplace add"),
            ("list", "Usage: elva plugin marketplace list"),
            ("upgrade", "Usage: elva plugin marketplace upgrade"),
            ("remove", "Usage: elva plugin marketplace remove"),
        ] {
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `elva mcp`"
        );
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `elva remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `elva app-server proxy`"
        );
    }
