...
pub struct Cli {
    ...
    // Internal controls set by the top-level `elva resume` subcommand.
    // These are not exposed as user flags on the base `elva` command.
    #[clap(skip)]
    ...
    /// top-level `elva resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `elva fork` subcommand.
    // These are not exposed as user flags on the base `elva` command.
    #[clap(skip)]
    ...
    /// top-level `elva fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
