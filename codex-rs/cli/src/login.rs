...
//! interactive-session layers. Direct `elva login` intentionally does less: it preserves the
...
/// Installs a small file-backed tracing layer for direct `elva login` flows.
///
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    // Direct `elva login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `elva login --device-auth` instead."
    );
}
...
/// `elva login` working in environments where device-code may be disabled/feature-gated.
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let codex_home = tempdir().expect("create temporary Elva home");
        login_with_api_key(
            codex_home.path(),
            "sk-existing",
